...
const SEARCH_MODE_CONFIG: Record<SearchMode, { placeholder: string; icon: React.ReactNode }> = {
  keyword: {
...
export function SemanticSearch() {
  ...
  useEffect(() => {
    ...
    const handleClickOutside = () => setShowModeDropdown(false);
    document.addEventListener('click', handleClickOutside);
    return () => document.removeEventListener('click', handleClickOutside);
  }, [showModeDropdown]);

  const currentConfig = SEARCH_MODE_CONFIG[searchMode];
  const currentLabel = SEARCH_MODE_LABELS[searchMode];

  return (
    <div className="relative flex items-center gap-2 flex-1">
      {/* Search mode selector - fixed width */}
      <div className="relative shrink-0">
        <button
          ...
          className="flex items-center justify-center gap-1.5 w-[100px] py-2 bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-md text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] hover:border-[var(--color-accent)] transition-colors text-xs font-medium"
          title={`Search mode: ${currentLabel}`}
        >
          {currentConfig.icon}
        ...
        {showModeDropdown && (
          <div
            className="absolute top-full left-0 mt-1 bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-md shadow-lg z-50 min-w-[140px]"
